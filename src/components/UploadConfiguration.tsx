...
/**
 * Reducer for managing upload configuration state
 */
function uploadConfigReducer(
  prev: UploadConfig,
  action: UploadConfigurationStateAction,
  autoTextTracks: NonNullable<UploadConfig['text_tracks']>,
...
/**
 * Custom hook for video validation and metadata extraction
 */
function useVideoValidation(
  ...
  useEffect(() => {
    ...
    const loadVideoMetadata = (videoSrc: string, shouldRevokeUrl = false) => {
      ...
      videoElement.onloadedmetadata = () => {
        ...
        // Detect if this is a video file (has video dimensions) or audio file (no video dimensions)
        const hasVideoDimensions = videoElement.videoWidth && videoElement.videoHeight
        setIsVideoFile(!!hasVideoDimensions)
        ...
        if (maxDuration && maxDuration > 0 && duration > maxDuration) {
          setValidationError(
            `Video duration (${formatSeconds(duration)}) exceeds maximum allowed duration of ${formatSeconds(maxDuration)}`
          )
        }
    ...
    const validateFileSize = (size: number): boolean => {
      ...
      setValidationError(
        `File size (${formatBytes(size)}) exceeds maximum allowed size of ${formatBytes(maxFileSize)}`
      )
      return false
    }
    ...
    if (stagedUpload.type === 'file') {
      ...
      // Check mime type to determine if it's a video or audio file
      ...
      const fetchFileSize = async () => {
        ...
        try {
          ...
          const shouldValidateDuration =
            maxFileSize === undefined || fileSize === null || validateFileSize(fileSize)

          if (fileSize === null && maxFileSize !== undefined) {
  ...
  }, [stagedUpload, maxFileSize, maxDuration])
...
function ValidationErrorDisplay({
  ...
  watermarkValidationError: string | null
}) {
  if (!validationError && !watermarkValidationError) return null
...
function FileUploadInfo({
  ...
  return (
    <>
      ...
      <Card tone="transparent" border padding={3} paddingY={4} style={{borderRadius: '0.1865rem'}}>
        <Flex gap={2}>
...
function StaticRenditionsConfig({
  ...
  id: string
  config: UploadConfig
  renditionMode: 'standard' | 'advanced'
  onModeChange: (mode: 'standard' | 'advanced') => void
  onToggleRendition: (rendition: StaticRenditionResolution) => void
}) {
  return (
    <FormField
        ...
        <Flex gap={3}>
          ...
          <Flex align="center" gap={2}>
            <Radio
              ...
              name="rendition-mode"
              onChange={() => onModeChange('advanced')}
              value="advanced"
              id={`${id}--mode-advanced`}
            />
          ...
          <Stack space={2}>
            <Flex align="center" gap={2} padding={[0, 2]}>
              <Checkbox
                ...
                onChange={() => onToggleRendition('highest')}
              />
            ...
            <Flex align="center" gap={2} padding={[0, 2]}>
              <Checkbox
                ...
                onChange={() => onToggleRendition('audio-only')}
              />
                ...
                return (
                  <Flex key={value} align="center" gap={2}>
                    <Checkbox
                      ...
                      onChange={() => onToggleRendition(value)}
                    />
...
export default function UploadConfiguration({
  ...
  secrets: Secrets
  pluginConfig: PluginConfig
  startUpload: (settings: MuxNewAssetSettings, watermark?: WatermarkConfig) => void
  onClose: () => void
}) {
  const id = useId()
  ...
  const [config, dispatch] = useReducer(
    (prev: UploadConfig, action: UploadConfigurationStateAction) =>
      uploadConfigReducer(prev, action, autoTextTracks, pluginConfig),
    {
  ...
  // Video validations using custom hook
  const {
    videoDuration,
    urlFileSize,
    isLoadingDuration,
    ...
    videoAspectRatio,
    isVideoFile,
  } = useVideoValidation(stagedUpload, pluginConfig.maxAssetFileSize, pluginConfig.maxAssetDuration)
  ...
  useEffect(() => {
    if (skipConfig) {
      const {settings, watermark} = formatUploadConfig(config, {videoAspectRatio})
    ...
    >
      <Stack padding={4} space={2}>
        <ValidationErrorDisplay
          ...
          watermarkValidationError={watermarkValidationError}
        />
        <FileUploadInfo
          stagedUpload={stagedUpload}
            ...
            {!basicConfig && (
              <FormField title="Additional Configuration">
                <Stack space={3}>
                  ...
                  <Stack space={3}>
                    <StaticRenditionsConfig
                      id={id}
                      config={config}
                      renditionMode={renditionMode}
                      onModeChange={handleModeChange}
                      onToggleRendition={toggleRendition}
                    />
                  </Stack>
                </Stack>
              </FormField>
