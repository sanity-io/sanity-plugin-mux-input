...
import {useSaveSecrets, useSecrets} from '../hooks/useSecrets'
...
function ConfigureApi({setDialogState}: Props) {
  ...
  const handleSubmit = useCallback(
    (event: React.FormEvent<HTMLFormElement>) => {
      ...
      if (!saving.current) {
        ...
        saveSecrets({token, secretKey, enableSignedUrls})
          .then(() => void unstable_batchedUpdates(() => setDialogState(false)))
          .catch(
            (err) =>
    ...
    [saveSecrets, setDialogState, state]
  )
  ...
  return (
    <Dialog id={id} header={header} width={1} onClose={handleClose}>
      <Box
        paddingRight={4}
        paddingLeft={4}
        paddingBottom={4}
